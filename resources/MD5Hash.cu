...
// source: https://github.com/vetter/shoc/blob/0aea03beba2f09fcb5935cc11737372fe4de9ec0/src/cuda/level1/md5hash/MD5Hash.cu
...
// Modifications:
// ****************************************************************************
__host__ __device__ void IndexToKey(uint64_t index,
...
// Modifications:
// ****************************************************************************
__device__ void FindKeyWithDigest_Kernel(dim3 blockIdx,
                                         ...
                                         int byteLength,
                                         int valsPerByte,
                                         lightning::Scalar<uint64_t> foundIndex
                                         //unsigned char *foundKey,
                                         //unsigned int *foundDigest
                                         )
{
    uint64_t threadid = (uint64_t)blockIdx.x * (uint64_t)blockDim.x + (uint64_t)threadIdx.x;
    uint64_t startindex = threadid * valsPerByte;
    unsigned char key[8] = {0,0,0,0, 0,0,0,0};
    IndexToKey(startindex, byteLength, valsPerByte, key);

    uint64_t local_foundIndex = !0;
    ...
    __shared__ uint64_t shared_foundIndex[block_size_x];
    shared_foundIndex[threadIdx.x] = !0;
    __syncthreads();

    if (threadIdx.x == 0) {
        for (int i = 0; i < blockDim.x; i++) {
            if (shared_foundIndex[i] != !0) {
                local_foundIndex = shared_foundIndex[i];
            }
        }

        *foundIndex = local_foundIndex;
    }
